...
ptmnb <- proc.time()
modelFitnb <- train(cleanTrain$classe ~ ., method='nb', data=pcaTrain)
timenb <- proc.time() - ptmnb
cMnb <- confusionMatrix(cleanTest$classe,predict(modelFitnb,pcaTest))

ptmlda <- proc.time()
modelFitlda <- train(cleanTrain$classe ~ ., method='lda', data=pcaTrain)
timelda <- proc.time() - ptmlda
cMlda <- confusionMatrix(cleanTest$classe,predict(modelFitlda,pcaTest))

ptmgbm <- proc.time()
modelFitgbm <- train(cleanTrain$classe ~ ., method='gbm', data=pcaTrain, verbose = FALSE)
timegbm <- proc.time() - ptmgbm
cMgbm <- confusionMatrix(cleanTest$classe,predict(modelFitgbm,pcaTest))
```
